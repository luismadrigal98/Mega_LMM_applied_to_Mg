run_CV_rrBLUP <- function(Y, fold_matrix, sample_data, K, formula)
{
  ...
  #' 
  #' @note The CV approach will be implemented in the classical way, in the sense
  #' that one fold will be left outside for testing in each iteration
  ...
  predictions <- Y * NA  # Same structure as input, but filled with NA
  predictions_train <- Y * NA  # For storing training predictions
  accuracies <- matrix(NA, n_folds, n_traits, 
                       dimnames = list(paste0("Fold_", folds), colnames(Y)))
  
  # Outer loop over folds - can be parallelized
  ...
  #                        sample_data_train <- sample_data[rownames(Y_train), ]
  #                        
  #                        # Run rrBLUP on training data
  #                        predictions_all <- run_rrBLUP_baseline(Y_train, sample_data_train, K, formula)
  #                        
  #                        # Store training predictions
  #                        fold_predictions_train[is_train, ] <- predictions_all
  #                        
  #                        # Prepare test data for prediction
  #                        sample_data_test <- sample_data[is_test, ]
  #                        
  ...
  #                        
  #                        # Get predictions for test samples for the current trait
  #                        res <- mixed.solve(y = Y_train[, trait_idx], X = X_train, K = K[rownames(Y_train), rownames(Y_train)])
  #                        test_preds <- c(X_test %*% res$beta) + res$u[match(rownames(sample_data_test), rownames(Y_train))]
  #                        
  #                        # Store test predictions
  #                        fold_predictions[is_test, trait_idx] <- test_preds
  #                        
  #                        # Calculate accuracy (correlation between observed and predicted)
  ...
  for(i in 1:n_folds)
  {
    ...
    for(trait_idx in 1:n_traits) {
      ...
      Y_train <- Y[is_train, , drop = FALSE]
      sample_data_train <- sample_data[rownames(Y_train), ]
      Ki <- K[rownames(Y_train), rownames(Y_train)]
  ...
  for(i in 1:n_folds) {
    ...
    predictions[!is.na(fold_result$predictions)] <- fold_result$predictions[!is.na(fold_result$predictions)]
    
    # Update training predictions 
    predictions_train[!is.na(fold_result$predictions_train)] <- 
      fold_result$predictions_train[!is.na(fold_result$predictions_train)]
